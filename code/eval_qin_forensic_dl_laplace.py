# eval_qin_forensic_dl_laplace.py
...
def dirichlet_harmonic_diffusion(prior_3d_logits, target_slices_2d, cx, cy, cz, num_iterations=100):
    """
    基于狄利克雷硬约束的 3D 拉普拉斯扩散
    - prior_3d_logits: [1, 1, X, Y, Z]
    - target_slices_2d: [3, 128, 128] (0: Axial/Z, 1: Coronal/Y, 2: Sagittal/X)
    """
    # 1. 转换 Target 为 Logit 空间
    target_logits_2d = torch.where(target_slices_2d > 0.5, 10.0, -10.0).float()

    # 2. 构造 3D 边界掩膜 (X, Y, Z 轴切片)
...
class QinForensicConfig:
    # 🔴 算法处决开关: 'difnet_laplace', 'difnet_tto', 'difnet_no_tto', '3dunet', 'swin_unetr'
...
if __name__ == '__main__':
    ...
    for idx in tqdm(range(len(val_dst)), ncols=100, desc="Forensic Autopsy"):
        ...
        elif QinForensicConfig.model_type == 'difnet_tto':
