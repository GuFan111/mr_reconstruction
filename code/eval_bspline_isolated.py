import os
import time
import numpy as np
import torch
import SimpleITK as sitk
from torch.utils.data import DataLoader
from monai.metrics import compute_hausdorff_distance
from tqdm import tqdm
import hashlib
import csv # 🟢 新增：用于结构化数据存储

from dataset import Prostate_Dataset
from utils import save_visualization_3view

# ==========================================
#  🟢 隔离指标计算逻辑
# ==========================================
def compute_isolated_dice(pred_np, gt_np, mask_purged):
    p = pred_np[mask_purged].astype(bool)
    g = gt_np[mask_purged].astype(bool)
    union = p.sum() + g.sum()
    if union == 0: return np.nan
    return 2.0 * np.logical_and(p, g).sum() / union

class Config:
    data_root = r'/root/autodl-tmp/Proj/data/prostate_158_128'
    out_res = (128, 128, 128)
    save_dir = './logs/baseline_bspline_isolated_extreme_local'
    # 🟢 新增：日志路径定义
    log_path = os.path.join(save_dir, 'autopsy_report.txt')
    csv_path = os.path.join(save_dir, 'per_case_results.csv')

# 🟢 新增：日志书写工具
def log_and_print(msg, log_file):
    print(msg)
    with open(log_file, 'a', encoding='utf-8') as f:
        f.write(msg + '\n')

def bspline_sparse_registration(fixed_img_np, moving_img_np, coords, grid_physical_spacing=30.0):
    fixed_image = sitk.GetImageFromArray(fixed_img_np.astype(np.float32))
    moving_image = sitk.GetImageFromArray(moving_img_np.astype(np.float32))

    cx, cy, cz = int(coords[0]), int(coords[1]), int(coords[2])
    mask_np = np.zeros_like(fixed_img_np, dtype=np.uint8)
    mask_np[cx, :, :] = 1
    mask_np[:, cy, :] = 1
    mask_np[:, :, cz] = 1
    metric_mask = sitk.GetImageFromArray(mask_np)
    metric_mask.CopyInformation(fixed_image)

    transformDomainMeshSize = [max(1, int(dim / grid_physical_spacing)) for dim in fixed_image.GetSize()]
    initial_transform = sitk.BSplineTransformInitializer(fixed_image, transformDomainMeshSize)

    registration_method = sitk.ImageRegistrationMethod()
    registration_method.SetMetricAsMeanSquares()
    registration_method.SetMetricFixedMask(metric_mask)
    registration_method.SetOptimizerAsLBFGSB(gradientConvergenceTolerance=1e-5,
                                             numberOfIterations=100,
                                             maximumNumberOfCorrections=5)
    registration_method.SetInitialTransform(initial_transform, inPlace=False)
    registration_method.SetInterpolator(sitk.sitkLinear)

    final_transform = registration_method.Execute(fixed_image, moving_image)
    return final_transform

if __name__ == '__main__':
    os.makedirs(Config.save_dir, exist_ok=True)
    os.makedirs(os.path.join(Config.save_dir, 'vis'), exist_ok=True)

    # 🟢 初始化结构化 CSV 文件
    with open(Config.csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Case_Name', 'Iso_Dice', 'Glo_HD95', 'Time_Sec'])

    log_and_print(f"[{time.ctime()}] Starting B-spline Isolated Forensic Test...", Config.log_path)

    print("Loading Test Dataset...")
    val_dst = Prostate_Dataset(data_root=os.path.join(Config.data_root, 'test', 'image'),
                               label_root=os.path.join(Config.data_root, 'test', 'label'),
                               npoint=1024, split='test', out_res=Config.out_res)
    eval_loader = DataLoader(val_dst, batch_size=1, shuffle=False)

    iso_recon_dices = []
    glo_recon_hd95s = []
    inference_times = []

    for i, item in enumerate(tqdm(eval_loader, ncols=100)):
        name = item['name'][0]

        seed_str = str(name) + "_fixed_seed"
        seed_val = int(hashlib.md5(seed_str.encode('utf-8')).hexdigest(), 16) % (2**32)
        np.random.seed(seed_val)
        torch.manual_seed(seed_val)

        target_img_np = item['target_image'][0, 0].numpy()
        prior_img_np = item['prior_image'][0, 0].numpy()
        prior_mask_np = item['prior_mask'][0, 0].numpy()
        target_mask_np = item['target_mask'][0, 0].numpy()
        coords = item['center_coords'][0].numpy()

        np.random.seed()
        torch.seed()

        t_start = time.time()
        try:
            final_transform = bspline_sparse_registration(target_img_np, prior_img_np, coords)
        except Exception as e:
            log_and_print(f"[{name}] FAILED: {e}", Config.log_path)
            continue

        prior_mask_sitk = sitk.GetImageFromArray(prior_mask_np.astype(np.float32))
        resampler = sitk.ResampleImageFilter()
        resampler.SetReferenceImage(prior_mask_sitk)
        resampler.SetInterpolator(sitk.sitkLinear)
        resampler.SetDefaultPixelValue(0)
        resampler.SetTransform(final_transform)

        pred_mask_sitk = resampler.Execute(prior_mask_sitk)
        pred_mask_np = sitk.GetArrayFromImage(pred_mask_sitk)
        pred_mask_np = (pred_mask_np > 0.5).astype(np.float32)

        duration = time.time() - t_start
        inference_times.append(duration)

        # 🔴 构建信息剥夺掩码
        cx, cy, cz = int(coords[0]), int(coords[1]), int(coords[2])
        M_anchor = np.zeros(Config.out_res, dtype=bool)
        M_anchor[cx, :, :] = True
        M_anchor[:, cy, :] = True
        M_anchor[:, :, cz] = True
        mask_purged = ~M_anchor

        # 结算
        iso_r_dice = compute_isolated_dice(pred_mask_np, target_mask_np, mask_purged)

        pred_mask_tensor = torch.from_numpy(pred_mask_np).unsqueeze(0).unsqueeze(0).cuda()
        gt_mask_tensor = torch.from_numpy(target_mask_np).unsqueeze(0).unsqueeze(0).cuda()

        try:
            glo_r_hd95 = compute_hausdorff_distance(pred_mask_tensor, gt_mask_tensor, include_background=False, percentile=95).item()
        except:
            glo_r_hd95 = 99.0

        iso_recon_dices.append(iso_r_dice)
        glo_recon_hd95s.append(glo_r_hd95)

        # 🟢 写入 CSV 与 TXT
        with open(Config.csv_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([name, iso_r_dice, glo_r_hd95, duration])

        if i % 5 == 0: # 减少 IO 频率，每5个 case 记一次详情
            log_and_print(f"  [{name}] Dice: {iso_r_dice:.3f} | HD95: {glo_r_hd95:.2f}mm | {duration:.2f}s", Config.log_path)

        if i == 0:
            vis_save_path = os.path.join(Config.save_dir, 'vis', f"bspline_{name}.png")
            save_visualization_3view(
                img_np=target_img_np, prior_mask=prior_mask_np, aligned_prior=prior_mask_np,
                gt_mask=target_mask_np, pred_mask=pred_mask_np, save_path=vis_save_path,
                case_name=name, epoch='Bspline'
            )

    # 🟢 总结报告
    report = f"""
{"="*60}
🔥 B-SPLINE FINAL AUTOPSY SUMMARY 🔥
{"="*60}
[Macroscopic Topology] Isolated Global Dice : {np.nanmean(iso_recon_dices):.4f}
[Macroscopic Topology] Global HD95 (mm)     : {np.nanmean(glo_recon_hd95s):.2f}
[Temporal Profiling]   Inference Time        : {np.mean(inference_times):.2f} s/vol
{"="*60}
"""
    log_and_print(report, Config.log_path)