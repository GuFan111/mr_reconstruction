...
class DIF_Net(nn.Module):
    ...
    def forward(self, data, is_eval=False, eval_npoint=100000):
        ...
        if not is_eval:
            ...
            for i in range(n_batch):
                ...
                p_pred_batch, _ = self.forward_points(
    ...
    def forward_points(self, proj_feats, prior_feats_vol, data, ms3dv_volumes=None):
        ...
        out = self.point_classifier(p_in).transpose(1, 2)
        delta_coords = out

        delta_coords = torch.tanh(delta_coords) * 0.2
        ...
        corrected_coords = data['points'].squeeze(1) + delta_coords
